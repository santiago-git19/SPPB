...
- Utilidades de DualOrbbecCapture o BagOrbbecCapture

Uso con cámaras en vivo:
...
tracker = Gait3DTracker(capture_source=dual, camera_side='left')

Uso con archivos .bag:
from utils.bag_orbbec_capture import BagOrbbecCapture
from utils.gait_3d_tracker import Gait3DTracker

bag_capture = BagOrbbecCapture("archivo.bag", enable_depth=True)
...
class Gait3DTracker:
    ...
    Parámetros:
    - capture_source: instancia de DualOrbbecCapture o BagOrbbecCapture con depth habilitado
    - camera_side: 'left' o 'right' para seleccionar la cámara usada para depth/intrínsecos (solo para DualOrbbecCapture)
    ...
    def __init__(
        self,
        capture_source: Union[Any, str, None] = None,
        ...
        dual_cam: Union[Any, str, None] = None,
    ) -> None:
        # Manejar compatibilidad hacia atrás
        if capture_source is None and dual_cam is not None:
            capture_source = dual_cam
        
        if capture_source is None:
            raise ValueError("capture_source no puede ser None")
        
        # Detectar el tipo de capture source
        self._detect_capture_mode(capture_source)
    ...
    def _detect_capture_mode(self, capture_source: Union[Any, str, None]):
        """
        Detectar el modo de captura (DualOrbbecCapture, BagOrbbecCapture, o modo especial).
        
        Args:
            capture_source: La fuente de captura a analizar
        """
        if isinstance(capture_source, str):
            if capture_source == "bag_file":
                self.capture_mode = "bag_file_legacy"
                return
            else:
                raise ValueError(f"Valor de string no reconocido: {capture_source}")
        
        # Detectar por nombre de clase
        class_name = capture_source.__class__.__name__ if hasattr(capture_source, '__class__') else str(type(capture_source))
        
        if class_name == "DualOrbbecCapture" or hasattr(capture_source, 'read_frames'):
            # Verificar que tenga depth habilitado
            if not hasattr(capture_source, 'enable_depth') or not capture_source.enable_depth:
                raise ValueError("DualOrbbecCapture debe tener depth habilitado")
            self.capture_mode = "dual_camera"
            
        elif class_name == "BagOrbbecCapture" or hasattr(capture_source, 'read_frame_with_depth'):
            # Verificar que tenga depth habilitado
            if not hasattr(capture_source, 'enable_depth') or not capture_source.enable_depth:
                raise ValueError("BagOrbbecCapture debe tener depth habilitado")
            self.capture_mode = "bag_file"
            
        else:
            raise ValueError(f"Tipo de capture_source no reconocido: {class_name}")

    @property
    def dual_cam(self):
        """Propiedad para mantener compatibilidad hacia atrás."""
        return self.capture_source

    @dual_cam.setter
    def dual_cam(self, value):
        """Setter para mantener compatibilidad hacia atrás."""
        self.capture_source = value
    ...
    def _region_depth_mm(self, depth_frame: np.ndarray, x: int, y: int) -> Optional[float]:
        """Obtener profundidad usando el método apropiado según el modo de captura."""
        half = self.region // 2
        
        # Usar método simple para modo legacy
        if self.capture_mode == "bag_file_legacy":
            return self._simple_region_depth(depth_frame, x, y)
        
        # Para modos modernos (bag_file y dual_camera), usar la API estándar
        if hasattr(self.capture_source, 'get_depth_statistics_in_region'):
            return self._get_region_depth_with_stats(depth_frame, x, y, half)
        else:
            return self._simple_region_depth(depth_frame, x, y)
    
    def _get_region_depth_with_stats(self, depth_frame: np.ndarray, x: int, y: int, half: int) -> Optional[float]:
        """Obtener profundidad usando estadísticas de región."""
        stats = self.capture_source.get_depth_statistics_in_region(
            depth_frame, x - half, y - half, self.region, self.region
        )
        ...
        return self.capture_source.get_distance_at_point(depth_frame, x, y)
    ...
    def _simple_3d_coordinates(self, x: int, y: int, z_mm: float) -> Optional[Tuple[float, float, float]]:
    ...
    def _get_3d_coordinates(self, depth_frame: np.ndarray, x: int, y: int, z_mm: float) -> Optional[Tuple[float, float, float]]:
        """Obtener coordenadas 3D usando el método apropiado según el modo de captura."""
        if self.capture_mode == "bag_file_legacy":
            return self._simple_3d_coordinates(x, y, z_mm)
        elif hasattr(self.capture_source, 'get_3d_coordinates'):
            return self.capture_source.get_3d_coordinates(depth_frame, x, y, self.camera_side)
        else:
            return self._simple_3d_coordinates(x, y, z_mm)
    ...
    def update(
        ...
        xyz_mm_pix = self._get_3d_coordinates(depth_frame, x, y, z_mm_region)
